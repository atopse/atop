...
func TestAgentOffline(t *testing.T) {
	...
	testCases := []struct {
		...
		{title: "参数IP不能为空", data: data{IP: ""}, want: &web.Response{Code: 500}},
		{title: "参数IP必须合法", data: data{IP: "127.0.0.555"}, want: &web.Response{Code: 500}},
		{title: "Agent不存在时无法更新", data: data{IP: "127.0.0.111"}, want: &web.Response{Code: 404}},
		{title: "Agent存在时更新成功", data: data{IP: ip}, want: &web.Response{Code: 200}},
	}
